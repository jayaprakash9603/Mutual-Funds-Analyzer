...
function StepUpMilestoneTooltip({
  active,
  payload,
  label,
}: {
  ...
  const duration = payload[0]?.payload?.duration as string | undefined
  const heading = duration ? `${label} · ${duration}` : label

  return (
    <div className="min-w-[9rem] rounded-lg border border-border bg-popover px-3 py-2 text-xs text-popover-foreground shadow-md">
      ...
      <div className="space-y-1">
        {payload.map((item) => {
          ...
          const seriesLabel = key === 'investment' ? 'Investment' : 'Returns'
    ...
    <div className={CHART_PANEL_CLASS}>
      ...
      >
        <BarChart data={chartData} margin={chartPlotMargin({ top: 8, bottom: 8 })}>
          ...
          <ChartTooltip cursor={CHART_TOOLTIP_CURSOR} content={<StepUpMilestoneTooltip />} />
          <Legend />
