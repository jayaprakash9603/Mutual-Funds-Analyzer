...
ALTER TABLE nav_point DROP CONSTRAINT IF EXISTS nav_point_pkey;
ALTER TABLE nav_point DROP COLUMN IF EXISTS id;
ALTER TABLE nav_point ADD PRIMARY KEY (scheme_code, series, nav_date);
CREATE INDEX idx_nav_point_covering ON nav_point (scheme_code, series, nav_date) INCLUDE (nav);
